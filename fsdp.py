...
# Training loop
def train(rank, model, device, train_loader, optimizer, criterion, epoch):
    model.train()
    train_loss = 0
    correct = 0
...
# Test loop
def test(rank, model, device, test_loader, criterion):
    model.eval()
    test_loss = 0
    correct = 0
    start_time = time.time()
...
def train_fsdp(rank, world_size):
    ...
    print("loaded train sampler")
    test_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset, num_replicas=world_size, rank=rank)
    print("loaded test sampler")
    
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=False, num_workers=16, sampler=train_sampler)
    print("loaded train loader")
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=16, sampler=test_sampler)
    ...
    print("model created")
    model = FSDP(model, cpu_offload=CPUOffload(offload_params=True), device_id=device, backward_prefetch=BackwardPrefetch.BACKWARD_PRE)
    ...
    for epoch in range(1, 11):
        ...
        train_load_time, train_push_time, train_time, total_train_time = train(rank, model, device, train_loader, optimizer, criterion, epoch)
        test_load_time, test_push_time, test_time, total_test_time = test(rank, model, device, test_loader, criterion)
        scheduler.step()
...
if __name__ == "__main__":
    ...
    spawn(train_fsdp, args=(world_size,), nprocs=world_size, join=True)